...
public class PowderedSnowTroll {
    ...
    public static void onPowderedSnowTroll(TickEvent.PlayerTickEvent event) {
        BlockState block = event.player.getLevel().getBlockState(new BlockPos(event.player.position()));
        if (block.is(Blocks.SNOW) && event.player.getRandom().nextInt(1000) <= AwfulnessConfig.POWDERED_SNOW_TROLL_THRESHOLD.get()) {
